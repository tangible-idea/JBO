...
- 자동 저장은 기본적으로 꺼져 있습니다. 설정에서 켜면 confidence 기준을 초과할 때만 자동 저장합니다.
- confidence가 저장한 기준 이하이거나 기존 폴더가 맞지 않으면, 편집 가능한 이름과 상위 폴더를 보여주고 새 폴더 생성과 북마크 저장을 한 번에 수행합니다.
- 설정 화면의 **기존 북마크 전체 정리**는 모든 URL 북마크의 페이지 제목과 메타 설명·키워드·사이트 이름을 병렬로 확인합니다. 완료된 20개 묶음부터 북마크 제목과 메타값을 JSON으로 JEV에 보내 분류합니다. 페이지를 읽을 수 없으면 저장된 제목과 URL로 분류합니다. 미리보기에서 선택한 항목만 `JEV 정리함/카테고리` 구조로 이동하며, 마지막 이동은 되돌릴 수 있습니다.
